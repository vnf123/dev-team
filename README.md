...
```mermaid
flowchart TD
    A[老板下达需求] --> B{PM 判断任务复杂度}
    B -->|简单/单领域| C[PM 直接完成]
    B -->|复杂/跨领域| D[PM 给老板简短计划]
    D --> E{老板确认?}
    E -->|否| F[PM 调整计划]
    F --> D
    E -->|是| G[PM 选择角色]
...
| 任务类型 | 默认并发上限 | 说明 |
|:---|:---:|:---|
| 简单任务 | 0 | 不派生 Agent，PM 直接完成 |
| 常规跨领域任务 | 2 | 通常是 UI + Backend，或 Frontend + Backend |
| 复杂全栈任务 | 3 | UI + Backend + DevOps / QA 中的一个 |
| 超大任务 | 先拆阶段 | 不建议一次性派 4+ Agent，容易浪费 Token 和产生冲突 |
...
| 问题 | 如果答案是「是」 | 动作 |
|:---|:---|:---|
| 只改一个文件/一个小点？ | 是 | PM 自己做 |
| 涉及 UI + 前端 + 后端？ | 是 | 启动团队 |
| 前端依赖设计稿或 API？ | 是 | 串行：UI/Backend → Frontend |
| UI 和后端互不依赖？ | 是 | 并行：UI + Backend |
| 需要部署/CI/CD？ | 是 | 加 DevOps |
| 改动跨多文件且风险高？ | 是 | 加 Reviewer |
| 业务规则复杂且容易漏测？ | 是 | 加 QA |
...
多 Agent 的好处是并行和专业分工，代价是 Token 增加。下面是经验参考，实际消耗会随项目大小、上下文长度、文件数量变化。
